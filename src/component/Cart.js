...
const Cart=()=>{
  ...
  return (
    <article>
      ...
      <h2 style={{color:"white" }}>My Cart Page</h2>
      {cart.map((item) => (
        <div className="cart_box" key={item.id}>
          ...
          <div style={{ textAlign: 'right'}}>
            ...
            <button  style={{color:"white",background:"red" , border:"1px solid transparent", margin:" 15px ",padding:"3px",borderRadius:"3px"}}onClick={() => handleRemove(item.id)}>Remove</button>
          </div>
        </div>
      ))}
      <div className="total">
